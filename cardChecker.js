...
// console logs, to check its working
...
const idInvalidCards = invalidBatch => {
  ...
  for (let i = 0; i < invalidBatch.length; i++) {
    switch (invalidBatch[i][0]){
      case 3:
      if(providers.indexOf('Amex') === -1){
        providers.push('Amex');
      }
      break
      case 4:
      if(providers.indexOf('Visa') === -1){
        providers.push('Visa');
      }
      break
      case 5:
      if(providers.indexOf('Mastercard') === -1){
        providers.push('Mastercard');
      }
      break 
      case 6:
      if(providers.indexOf('Discover') === -1){
...
/* logs to check providers
console.log(idInvalidCards([invalid1])); 
console.log(idInvalidCards([invalid2]));
console.log(idInvalidCards(batch)); 
*/
